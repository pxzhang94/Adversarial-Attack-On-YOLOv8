...
def run(input_folder):
    ...
    folder = Path(input_folder) / 'results_test'
    ...
    for condition, results in summary.items():
        ...
        for img_path in img_list:
            diff_image_folder = diff_folder / Path(img_path).stem
            ...
            adv_image_path = folder / condition / 'images' / (Path(img_path).stem +'.png')
            ...
            # 保存图片（保持原始文件名）
            out_original_path = diff_image_folder / 'original.png'
            out_adv_path = diff_image_folder / 'adversarial.png'
