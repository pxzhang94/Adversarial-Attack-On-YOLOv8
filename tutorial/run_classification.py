...
def batch_inference(model, img_paths, input_size, output_label_path, normalize=None, mean=None, std=None):
...
def run(model_file=None, input_folder=None, input_size=None, normalize=None, mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0], mode='upload', model_desc_file_no=None):
    ...
    # 1. 预测原始图像
    batch_inference(model, img_paths, input_size, original_pred_path, normalize, mean, std)
        ...
        for transform in transforms:
            ...
            for factor in transform_factors:
                ...
                if transform in ['fgsm', 'pgd', 'cw']:
                    ...
                    transformed_img = transform_function(model, image, label, factor, normalize, mean, std)
                else:
    ...
    for transform_folder in transform_folders:
        ...
        batch_inference(model, transform_img_paths, input_size, transform_pred_path, normalize, mean, std)
...
if __name__ == "__main__":
    run(model_file='./vgg_cifar10_best.pt', input_folder="./demo_classification", normalize='CIFAR10', model_desc_file_no=1768547474, input_size=(32,32))
